//! pbc64 instruction arguments.

use super::*;
use crate::ir::condcodes::CondCode;
use crate::machinst::{abi::StackAMode, isle::WritableReg};
...
macro_rules! newtype_of_reg {
    (
        ...
        impl From<$newtype_reg> for cranelift_pulley::regs::$newtype_reg {
            fn from(r: $newtype_reg) -> Self {
                Self::new(r.to_real_reg().unwrap().hw_enc()).unwrap()
            }
        }
        impl<'a> From<&'a $newtype_reg> for cranelift_pulley::regs::$newtype_reg {
            fn from(r: &'a $newtype_reg) -> Self {
                Self::new(r.to_real_reg().unwrap().hw_enc()).unwrap()
            }
        }
        impl From<$newtype_writable_reg> for cranelift_pulley::regs::$newtype_reg {
            fn from(r: $newtype_writable_reg) -> Self {
                Self::new(r.to_reg().to_real_reg().unwrap().hw_enc()).unwrap()
            }
        }
        impl<'a> From<&'a $newtype_writable_reg> for cranelift_pulley::regs::$newtype_reg {
...
impl Amode {
    ...
    pub(crate) fn get_label_with_sink(&self, sink: &mut MachBuffer<Inst>) -> Option<MachLabel> {
        match self {
            Amode::SpOffset { .. } | Amode::RegOffset { .. } | Amode::Stack { .. } => None,
        }
    }
}
