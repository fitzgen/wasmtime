//! Implementation of a standard Pbc64 ABI.

use crate::{
    ir::{self, types::*, LibCall, MemFlags, Signature},
    isa::{
        self,
        pbc64::{inst::*, settings::Flags as PbcFlags},
        unwind::UnwindInst,
        CallConv,
    },
    machinst::*,
...
/// Support for the Pbc64 ABI from the callee side (within a function body).
pub(crate) type Pbc64Callee = Callee<Pbc64MachineDeps>;

/// Support for the Pbc64 ABI from the caller side (at a callsite).
pub(crate) type Pbc64ABICallSite = CallSite<Pbc64MachineDeps>;
...
/// Pbc64-specific ABI behavior. This struct just serves as an implementation
/// point for the trait; it is never actually instantiated.
pub struct Pbc64MachineDeps;

impl IsaFlags for PbcFlags {}

impl ABIMachineSpec for Pbc64MachineDeps {
    type I = Inst;
    type F = PbcFlags;
    ...
    fn gen_clobber_save(
        ...
        if stack_size > 0 {
            ...
            let sp_adj = i32::try_from(frame_layout.outgoing_args_size).unwrap();
            if sp_adj > 0 {
