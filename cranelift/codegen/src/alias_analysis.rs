...
fn observe(
    func: &Function,
    observed_stores: Observations<'_>,
    slot: Option<LastStore>,
    observer: Inst,
) {
    ...
    let Some(last_store) = slot.and_then(|s| s.inst()) else {
        return;
    };
...
///
/// `Option<LastStore>` is a simple meet semi-lattice:
///
/// ```ignore
///       Some(NoStore)   Some(Inst(i))
///                \        /
///                 \      /
///                  \    /
///                   None
...
enum LastStore {
    ...
    ///
    /// XXX: This is _not_ the same as `None`! This is *positive* information
    /// that nothing has been written since the last fence; in contrast, `None`
    /// is the *absence* of information. It is sound to fall back to the last
    /// fence from `NoStore`; it is unsound to do that from `None`.
    #[default]
    NoStore,
...
impl LastStore {
    ...
    fn meet(a: Option<Self>, b: Option<Self>) -> Option<Self> {
        if a == b { a } else { None }
    }
...
struct LastStores {
    ...
    regions: SecondaryMap<AliasRegion, Option<LastStore>>,
...
impl LastStores {
            ...
            if let Some(memflags) = func.dfg.insts[inst].memflags() {
                match func.dfg.mem_flags[memflags].alias_region() {
                    Some(region) => {
                        ...
                        self.regions[region] = Some(inst.into());
    ...
    fn observe_trapping_others(
        ...
        let can_trap = |slot: Option<LastStore>| {
            slot.is_some_and(|s| {
                s.inst()
        ...
        for (region, slot) in self.regions.iter() {
            if region != excluding && can_trap(*slot) {
                observe(func, observed_stores.as_deref_mut(), *slot, observer);
            }
        }
    ...
    fn fence(&mut self, func: &Function, inst: Inst, mut observed_stores: Observations<'_>) {
        ...
        // region.
        self.last_fence = Some(inst.into());
    }

    /// Get the last store to the given alias region.
    fn last_store_for_region(&self, region: AliasRegion) -> Option<LastStore> {
        match self.regions[region] {
            ...
            Some(LastStore::NoStore) => self.last_fence,
            slot => slot,
        }
    }
    ...
    fn raw_region_slot(&self, func: &Function, inst: Inst) -> Option<Option<LastStore>> {
    ...
    /// observed.
    fn undo_store(&mut self, func: &Function, dead: Inst, prev_region_slot: Option<LastStore>) {
        ...
        // region.
        if self.regions[region] == Some(dead.into()) {
            self.regions[region] = prev_region_slot;
        }
    }

    /// Get the last store to the given `inst`'s alias region.
    fn get_last_store(&self, func: &Function, inst: Inst) -> Option<LastStore> {
        if let Some(memflags) = func.dfg.insts[inst].memflags() {
    ...
    fn meet_from(&mut self, rhs: &LastStores) -> bool {
        ...
        let mut meet = |a: &mut Option<LastStore>, b: Option<LastStore>| {
...
struct KnownValue {
    ...
    prev_region_slot: Option<Option<LastStore>>,
}
    ...
    fn compute_observed_stores(&mut self, func: &Function) {
        ...
        for block in func.layout.blocks() {
            ...
            visit_block_succs(func, block, |_inst, succ, _from_table| {
                ...
                for i in 0..max_len {
                    let region = AliasRegion::new(i);
                    if succ_input.regions[region].is_none() {
                        observe(
                            func,
                ...
                if succ_input.last_fence.is_none() {
    ...
    pub fn process_inst(
        ...
        let result = if let Some((address, offset, ty)) = inst_addr_offset_type(func, inst) {
            ...
            if opcode.can_store() {
                ...
                if let Some(LastStore::Inst(last_store)) = last_store {
                ...
                let Some(last_store) = state.stores.get_last_store(func, inst) else {
                    return OptResult::None;
                };
