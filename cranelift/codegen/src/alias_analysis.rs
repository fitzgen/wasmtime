...
impl LastStores {
    fn update(&mut self, func: &Function, inst: Inst) {
        ...
        if has_memory_fence_semantics(opcode) {
            ...
            // Also record the fence for regions not yet in the map.
    ...
    fn meet_from(&mut self, other: &LastStores, loc: Inst) {
        ...
        // Meet all region slots. SecondaryMap auto-extends, so iterating
        // over the max of both maps' lengths is handled by indexing.
