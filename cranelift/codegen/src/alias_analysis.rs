...
/// Where the last-store transfer function records the stores it finds are
/// observed.
///
/// This is `None` while the last-store fixpoint is being solved: which stores
/// are observed must not depend on the order in which the worklist visits
/// blocks, so it is computed separately, from the solved fixpoint, in
/// `AliasAnalysis::compute_observed_stores`.
type Observations<'a> = Option<&'a mut FxHashMap<Inst, Observer>>;

/// Mark the store, if any, named by the given slot value as observed.
fn observe(func: &Function, observed_stores: Observations<'_>, slot: LastStore, observer: Inst) {
    let Some(observed_stores) = observed_stores else {
        return;
    };
...
/// ```
///
/// This is similar to `MemoryVersion` but provides a single, canonical bottom
/// element, rather than multiple pseudo-bottom elements that carry
/// identity. The single, canonical bottom element is what makes the
/// `LastStores` lattice fixpoint order-independent: if we instead use
/// `MemoryVersion` and its many "bottoms", then the descent path through the
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq, Hash)]
enum LastStore {
    /// Nothing has written to this region since the last fence (if any).
    ///
    /// XXX: This is _not_ the same as `Unknown`! This is *positive* information
    /// that nothing has been written since the last fence; in contrast,
    /// `Unknown` is the *absence* of information. It is sound to fall back to
    /// the last fence from `NoStore`; it is unsound to do that from `Unknown`.
    #[default]
    NoStore,
...
impl LastStore {
    ...
    ///
    /// Returns their greatest lower bound (GLB): the most precise value that is
...
/// The identity of memory region's version, used to key a `MemoryLoc`.
///
/// Two program points that agree on a region's `MemoryVersion` are guaranteed
/// to see the same bytes in that region.
///
/// Unlike `LastStore`, this is not a lattice: it has no single, canonical
/// bottom. Instead, it gives us identity; see the comments on `Merge(_)` below.
...
enum MemoryVersion {
    ...
    /// The memory version at the control-flow join generated by the given
    /// `Block`.
    ///
    /// Memory versions disagreed flowing into that block, so we can't say
    /// anything concrete about this memory region from before the join point,
    /// and nothing has written to the region since then.
    ///
    /// This gives us the exact same (lack of) information about the region's
    /// contents that `LastStore::Unknown` does. However, it additionally gives
    /// us identity: by naming the block where we lost information, different
    /// join points get different `MemoryLoc` keys, which correctly prevents us
    /// from, e.g., forwarding the value loaded after one join point to a load
    /// after a different join point.
    Merge(Block),
}

impl From<Inst> for MemoryVersion {
    #[inline]
    fn from(inst: Inst) -> Self {
        Self::Inst(inst)
    }
...
impl From<Block> for MemoryVersion {
    #[inline]
...
impl LastStores {
    fn update(&mut self, func: &Function, inst: Inst, mut observed_stores: Observations<'_>) {
    ...
    fn observe_others(
        &self,
        func: &Function,
        mut observed_stores: Observations<'_>,
        excluding: Option<AliasRegion>,
        observer: Inst,
    ) {
        if observed_stores.is_none() {
            return;
        }

        for (region, version) in self.regions.iter() {
            if excluding.is_none_or(|r| r != region) {
                observe(func, observed_stores.as_deref_mut(), *version, observer);
            }
        }
    ...
    fn observe_trapping_others(
        ...
        for (region, version) in self.regions.iter() {
            if region != excluding && can_trap(*version) {
                observe(func, observed_stores.as_deref_mut(), *version, observer);
            }
        }
    ...
    fn fence(&mut self, func: &Function, inst: Inst, mut observed_stores: Observations<'_>) {
        // A fence can observe every region, so every store we are currently
        // tracking for a region becomes observed.
        if observed_stores.is_some() {
            for (_region, version) in self.regions.iter() {
                observe(func, observed_stores.as_deref_mut(), *version, inst);
            }
        }
...
pub struct MemoryState {
    ...
    /// The block whose `MemoryVersion::Merge` token stands for this state's
    /// `LastStore::Unknown` slots.
    ///
    /// This is not necessarily the block we are currently walking. It is the
    /// nearest ancestor reachable by walking up single-predecessor edges. See
    /// `AliasAnalysis::compute_merge_blocks`.
    merge_block: Block,
...
impl MemoryState {
    /// Convert a region's `LastStore` value into the memory version identity to
    /// key a `MemoryLoc` on.
    fn version(&self, slot: LastStore) -> MemoryVersion {
        match slot {
            LastStore::NoStore => MemoryVersion::NoStore,
            LastStore::Inst(i) => i.into(),
            LastStore::Unknown => self.merge_block.into(),
        }
    }
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
struct MemoryLoc {
    /// The version of memory that this location's contents belong to.
...
pub struct AliasAnalysis<'a> {
    ...
    /// Which block generates the `MemoryVersion::Merge` token that identifies a
    /// given block's `LastStore::Unknown` slots.
    ///
    /// A missing entry means that a block is its own merge block, which is the
    /// common case, so this map stays small.
    merge_blocks: FxHashMap<Block, Block>,

    /// Known memory-value equivalences.
    ///
    /// This is the result of the analysis. This is a mapping from a memory
    /// location to its known value.
...
impl<'a> AliasAnalysis<'a> {
    ...
    fn compute_block_input_states(&mut self, func: &Function) {
        ...
        // NB: we deliberately avoid observng stores while solving the
        // fixpoint. A `LastStore` slot can transiently name a store that the
        // final fixpoint does not, so observing them here would make the
        // observed-stores set depend on the order in which we happen to visit
        // blocks. Instead, we compute the observed-stores set after the
        // fixpoint; see `compute_observed_stores`.
        let mut observed_stores: Observations<'_> = None;
        ...
        while let Some(block) = queue.pop() {
            ...
            for inst in func.layout.block_insts(block) {
                ...
                state.update(func, inst, observed_stores.as_deref_mut());
    ...
    /// Compute each reachable block's merge block.
    ///
    /// The merge block is the first predecessor (including the block itself)
    /// that is a control-flow join point or the entry block.
    ///
    /// This lets us identify different `MemoryVersion::Merge(_)`s and
    /// distinguish them from `LastStore::Unknown` inside a `MemoryLoc`.
    fn compute_merge_blocks(&mut self, func: &Function, cfg: &ControlFlowGraph) {
        let entry = func.layout.entry_block().unwrap();

        // Set the merge block of the whole single-predecessor chain for each
        // block as we go, union-find style, to avoid quadratic behavior.
        let mut chain = vec![];
        ...
        for block in func.layout.blocks() {
            ...
            let merge_block = loop {
                // If we've already computed the merge block for `cursor`, then
                // reuse it.
                if let Some(merge_block) = self.merge_blocks.get(&cursor) {
                    break *merge_block;
                }

                // The entry block is always its own merge block.
                if cursor == entry {
                    break entry;
                }

                match self.single_reachable_pred(cfg, cursor) {
                    // Join point: we found the merge block!
                    None => break cursor,
                    // Not a join point: push it onto the chain and continue the
                    // loop.
                    Some(pred) => {
                        assert!(chain.len() < self.block_input.len());
                        chain.push(cursor);
                        cursor = pred;
                    }
                }
            ...
            // The whole chain has the same merge block.
            for b in chain.drain(..) {
                self.merge_blocks.insert(b, merge_block);
            }
        }
    }
    ...
    /// observe.
    ///
    /// This is deliberately not folded into the last-store fixpoint. While
    /// that fixpoint iterates, a slot can transiently name a store that the
    /// final solution does not, and observing it would make the result depend on
    /// the order in which the worklist visited blocks. Instead we walk the
    /// function once, in layout order, over the solved fixpoint.
    fn compute_observed_stores(&mut self, func: &Function) {
        ...
        for block in func.layout.blocks() {
            ...
            let mut state = self.block_starting_state(block).stores;
            for inst in func.layout.block_insts(block) {
                state.update(func, inst, Some(&mut observed_stores));
            }
            ...
            visit_block_succs(func, block, |_inst, succ, _from_table| {
                ...
                if succ_input.last_fence == LastStore::Unknown {
                    observe(func, Some(&mut observed_stores), state.last_fence, observer);
                }
            });
        }
    ...
    /// Get the initial memory state for a given block.
    pub fn block_starting_state(&self, block: Block) -> MemoryState {
        let stores = match self.block_input.get(&block) {
            Some(input) => input.clone(),
            None => LastStores::default(),
        };

        let merge_block = self.merge_blocks.get(&block).copied().unwrap_or(block);
    ...
    ///
    /// Must be invoked in program order within a block, and ideally in RPO
    /// across blocks or at least some domtree preorder for maximal reuse.
    pub fn process_inst(
        &mut self,
        func: &mut Function,
        cfg: &ControlFlowGraph,
        ...
        let result = if let Some((address, offset, ty)) = inst_addr_offset_type(func, inst) {
            ...
            if opcode.can_store() {
                ...
                let check_loc = MemoryLoc {
                    version: state.version(last_store),
                    address,
                    offset,
                    ty,
                    extending_opcode: get_ext_opcode(opcode),
        ...
        state
            .stores
            .update(func, inst, Some(&mut self.observed_stores));
        debug_assert_eq!(
            observed_stores_len,
