...
declare_builtin_functions! {
    ...
    /// Returns an index to drop a `VMExternRef` in place.
    drop_externref_in_place(pointer) -> ();
...
impl<'module_environment> cranelift_wasm::FuncEnvironment for FuncEnvironment<'module_environment> {
    ...
    fn make_table(&mut self, func: &mut ir::Function, index: TableIndex) -> WasmResult<ir::Table> {
        ...
        let element_size = u64::from(self.reference_type(WasmType::ExternRef).bytes());
    ...
    fn translate_table_set(
        ...
        match plan.table.wasm_ty {
            ...
            WasmType::ExternRef => {
                ...
                //    safety, but is required to avoid leaks.
                ...
                builder
                    .func
                    .layout
                    .insert_block_after(dec_ref_count_block, current_block);
                builder
                    .func
                    .layout
                ...
                // Increment `value`'s ref count. This has to come before decrementing
                // the current table element's ref count, or else we might reach ref
                // count == zero and deallocate the current table element, but if
                // `value` *is* the current element (and this `table.set` is therefore a
                // no-op), then we would leave a dangling pointer in the table, leading
                // to use after free.
                ...
                let current_elem_is_null = builder.ins().is_null(current_elem);
                builder
                    .ins()
                ...
                // Call the `drop_externref_in_place` builtin to (you guessed
                // it) drop the `externref` in place.
                builder.switch_to_block(drop_block);
                let builtin_idx = BuiltinFunctionIndex::drop_externref_in_place();
                let builtin_sig = self
                    .builtin_function_signatures
                    .drop_externref_in_place(builder.func);
                let (_vmctx, builtin_addr) = self
                    .translate_load_builtin_function_address(&mut builder.cursor(), builtin_idx);
                builder
                    .ins()
                    .call_indirect(builtin_sig, builtin_addr, &[table_entry_addr]);
                builder.ins().jump(continue_block, &[]);
