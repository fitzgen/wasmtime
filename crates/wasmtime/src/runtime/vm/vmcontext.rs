            ...
            match wasm_ty {
                ...
                WasmValType::Ref(r) => match r.heap_type.top() {
                    WasmHeapTopType::Extern => {
                        let r = VMGcRef::from_raw_ne_u32(raw.get_externref());
                        global.init_gc_ref(store, r.as_ref())
                    }
                    WasmHeapTopType::Any => {
                        let r = VMGcRef::from_raw_ne_u32(raw.get_anyref());
                        global.init_gc_ref(store, r.as_ref())
                    }
                    ...
                    WasmHeapTopType::Exn => {
                        let r = VMGcRef::from_raw_ne_u32(raw.get_exnref());
                        global.init_gc_ref(store, r.as_ref())
                    }
                },
            }
            ...
            Ok(match wasm_ty {
                ...
                WasmValType::Ref(r) => match r.heap_type.top() {
                    WasmHeapTopType::Extern => ValRaw::externref(match self.as_gc_ref() {
                        Some(r) => store.clone_gc_ref(r).as_raw_ne_u32(),
                        None => 0,
                    }),
